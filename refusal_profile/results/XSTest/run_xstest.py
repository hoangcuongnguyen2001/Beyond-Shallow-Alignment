...
# explicitly disallows it. Llama-3.1 benefits from it to avoid degenerate
# repetition loops in short greedy outputs.
...
def load_model_and_tokenizer(model_path: str, use_auth_token: bool = False):
    ...
    device_map = "cuda:0" if _is_gemma(model_path) else "auto"
    ...
    print(f"Model loaded. Device: {effective_device}")
    return model, tokenizer


def _format_prompt(tokenizer, prompt: str, system_prompt: str, is_gemma: bool) -> str:
...
def generate_responses(
    ...
    if tokenizer.chat_template is None:
        ref_model = "google/gemma-2-9b-it" if is_gemma else "meta-llama/Llama-3.1-8B-Instruct"
        raise ValueError(
            f"tokenizer.chat_template is None for {type(model).__name__}. "
            f"Fix with: AutoTokenizer.from_pretrained('{ref_model}').chat_template "
            f"saved into your checkpoint."
        )

    # Format all prompts up front
    texts = [_format_prompt(tokenizer, p, system_prompt, is_gemma) for p in prompts]
    ...
    gen_config = GENERATION_CONFIG_GEMMA if is_gemma else GENERATION_CONFIG_LLAMA
...
def compare_models(
    ...
    dataset = load_dataset(XSTEST_DATASET, split="test")
